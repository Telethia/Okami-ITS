...
def test():
    ...
    while i <= Entries - 1:
        ...
        CoordsX = int.from_bytes(ITSFile.read(2), 'little', signed=True)
        CoordsY = int.from_bytes(ITSFile.read(2), 'little', signed=True)
        CoordsZ = int.from_bytes(ITSFile.read(2), 'little', signed=True)
